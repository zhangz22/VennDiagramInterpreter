...
class ExpressionSet(object):
    ...
    def create_diagram(self):
        ...
        for exp in self.relations:
            ...
            if exp.symbol1.all:
                for area_label in support:
                    if area_label not in self.definite:
    ...
    def display_diagram(self, possible=True, definite=False):
        """
        This function displays the diagram according to the area codes generated by
        create_diagram() function.
        :param possible: a flag to determine whether to highlight "Some" preconditions
        ...
        area_colors, texts = [], []
        if definite:
            # Hightlight "All" preconditions using a background color
            for area_label, exps in self.definite.items():
                for exp in exps:
    ...
    def evaluate(self, exp: Expression, show=False, print_log=False):
        ...
        def mark_green(area: set, color="green"):
            if show:
        ...
        def mark_red(area: set, color="red"):
            if show:
                for area_label in area:
                    self.diagram.get_patch_by_id(
                        self.label_id[area_label]).set_edgecolor(color)
        ...
        if len(valid_against_area) != 0:
            ...
            if len(valid_support_area) != 0:
                ...
                definite_support_area = valid_support_area.intersection(self.definite.keys()) - self.possible.keys()
